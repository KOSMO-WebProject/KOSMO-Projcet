...
const getBackPackList = async (req, res) => {
    try {
        const [rows] = await db.get().execute("SELECT p.product_no, p.name, p.price, p.stock, p.img_url, p.category_no FROM product p inner join category c on p.category_no = c.category_no where c.parent_category = 25");
        return res.status(200).json(rows);
    } catch (error) {
        console.error(error);
