...
(async () => {
  ...
  const url = "https://www.musinsa.com/main/musinsa/ranking?storeCode=musinsa&sectionId=200&categoryCode=002012&gf=F";
  await page.goto(url, {
  ...
  await page.waitForSelector("#commonLayoutContents > article", {
    ...
    $(".sc-1y072n9-0.jdzDMq").each((index, element) => {
      ...
      const productPrices = $(element)
          ...
          .map((i, el) => {
            const productPriceText = $(el).text().trim().replace(/[^0-9]/g, ""); // 숫자만 남김
      ...
      productNames.forEach((name, i) => {
        ...
        products.push({product_no : count,name, price, stock: 50, img_url, category_no: 24});
        count++;
        productCount++;
      });
    });
